...
static void output_thread(void *data)
{
     ...
     */
    mk_event_channel_destroy(th_ins->evl, th_ins->ch_thread_events[0], th_ins->ch_thread_events[1], &event_local);
...
void flb_output_thread_pool_destroy(struct flb_output_instance *ins)
{
    ...
    mk_list_foreach(head, &tp->list_threads) {
        ...
        mk_event_channel_destroy(th_ins->evl, th_ins->ch_parent_events[0], th_ins->ch_parent_events[1], th_ins);
        flb_free(th_ins);
    }
